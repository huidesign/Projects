...
throw (
  ...
  RTI::FederateInternalError)
{   // Well, this callback function is not supported in BH-RTI
    //if (theClass == CFellow::GetFellowRtiId())
    //{
    //    CFellow::SetRegistration(RTI::RTI_TRUE);
    //    //AfxMessageBox(_T("startRegistrationForObjectClass"));
    //}
    //else
    //{
    //    //CString str;
    //    //str.Format(_T("startRegistrationForObjectClass unknown class: %ld"), 
    //    //    theClass);
    //    //AfxMessageBox(str);
    //}
}
  ...
  RTI::FederateInternalError)
{
    if (theClass == CFellow::GetFellowRtiId())
    {
        CFellow::SetRegistration(RTI::RTI_FALSE);
    }
}
  ...
  RTI::FederateInternalError)
{
   ...
   //}
    //AfxMessageBox((CString)theObjectName);
}
  ...
  RTI::FederateInternalError)
{
   ...
   ////-----------------------------------------------------------------
   //CFellow *pCFellow = CFellow::Find(theObject);
   ...
   //   //-----------------------------------------------------------------
   //   pCFellow->Update(theAttributes);
   //   //pCFellow->SetLastTime(theTime);
   //}
   ////else
  ...
  RTI::FederateInternalError)
{
   ...
   // so that it can be processed.
    char message[1000];
    RTI::ULong length;
    if (theInteraction == CCommunication::GetCommRtiId())
    {
        for (unsigned int idx = 0; idx < theParameters.size(); idx++)
        {
            theParameters.getValue(idx, (char*)message, length);
            m_pCChatResearchDlg->ReceiveCommMsg(message);
        }
    }
