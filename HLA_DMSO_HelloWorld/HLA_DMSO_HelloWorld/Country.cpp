...
Country::Country()
	: m_name(NULL),
	  ...
	  m_timePlusLookahead(*(RTI::FedTimeFactory::makeZero()))
{
	Country::ms_countryExtent[ ms_extentCardinality++ ] = this;
...
Country::~Country() 
{
	...
	// zeroth position always hold the local instance.
	//-----------------------------------------------------------------
...
void Country::PublishAndSubscribe()
{
	if (ms_rtiAmb)
	{
		...
		delete countryAttributes;
		//------------------------------------------------------
		// Same as above for interactions
		//------------------------------------------------------
	
		// Get RTTI info
		ms_commTypeId    = ms_rtiAmb->getInteractionClassHandle(ms_countryTypeStr);
	...
	for (unsigned int i = 0; i < theAttributes.size(); i++)
	{
		...
		if (attrHandle == Country::GetPopulationRtiId())
		{
			...
			*(((long*)&population) + 1) = x;
			...
			double x;
			cvt_ftof(&population, CVT_IEEE_T, &x, CVT_BTG_ENDIAN_IEEE_T, 0);
			population = x;
#endif
